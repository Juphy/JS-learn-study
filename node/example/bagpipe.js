 ...
 * ```
 * Events:
 * - 'full'，当活动异步达到限制值时，后续异步调用将被暂存于队列中。当队列的长度大于限制值的2倍或100的时候，触发'full'事件。事件传递队列长度值。
 * - 'outdated'，超时后的异步调用异常返回
 * Options：
 * - 'disabled'，禁用限流
 * - 'refuse'，拒绝模式，排队超过限制值时，新来的调用将会得到`ToomuchAsyncCallError`异常
 * - 'timeout'，设置异步调用的时间上限，保证异步调用能够恒定的结束，不至于话费太长时间
...
class BagPipe extends EventEmitter {
    ...
    push(method, ...args) {
        ...
        if (this.queue.length < this.queueLength || !this.options.refuse) {
            ...
            err.name = 'Too Much Async Call Error';
            callback(err);
        }
